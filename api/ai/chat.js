...
export default async function handler(request, response) {
  ...
  const instructions = `You are Mediatrix AI, an AI sales assistant. ${MEDIATRIX_KNOWLEDGE} Always reply in the website's selected language (${language}), even if the visitor writes in another language. Be friendly, concise, and ask at most one relevant question. Never claim to be human. For uncertainty, binding price/deadline, complex request, or serious hiring intent, set handoff true. Return JSON only: {"reply":"...","handoff":boolean,"language":"${language}"}.`;
  try {
    ...
    return response.status(200).json({ reply: parsed.reply.trim(), handoff: Boolean(parsed.handoff), language });
