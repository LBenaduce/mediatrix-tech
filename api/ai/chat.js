...
function languageFor(text, fallback) { const rules = [["pt-BR", /\b(olá|site|para|preciso|quero|orçamento|você)\b/i], ["it", /\b(ciao|sito|vorrei|sono|per|un)\b/i], ["es", /\b(hola|sitio|quiero|para|necesito)\b/i], ["fr", /\b(bonjour|site|pour|je|vous)\b/i], ["de", /\b(hallo|website|ich|und|für|möchte)\b/i]]; return rules.find(([, regex]) => regex.test(text))?.[0] || fallback; }
...
export default async function handler(request, response) {
  ...
  const instructions = `You are Mediatrix AI, an AI sales assistant. ${MEDIATRIX_KNOWLEDGE} Reply in ${language}. Be friendly, concise, and ask at most one relevant question. Never claim to be human. For uncertainty, binding price/deadline, complex request, or serious hiring intent, set handoff true. Return JSON only: {"reply":"...","handoff":boolean,"language":"${language}"}.`;
  try {
